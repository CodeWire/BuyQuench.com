...
<meta charset="utf-8">
    ...
    <!--<link href="<?php echo base_url(); ?>assets/css/footable.standalone.css" rel="stylesheet">-->
    <style>
	.table-responsive{width: auto;overflow-y: visible;overflow-x: visible; border:none;}
	.footable-row-detail-name{font-weight:bold;}
	</style>
  </head>

  <body>
  <?php if($this->session->userdata('temp_login')){
					echo '<a class="backtoadmin" href="'.base_url().'admin/customers/backtoadmin">Back to Admin</a>';
		} ?>
      <?php $currentParent = $this->router->fetch_class();
$currentChild = $this->router->fetch_method(); ?>
  <?php if($currentParent != "index"){ ?>
    <div id="header">
	...
	<div id="content">
      ...
      <?php $this->load->view($body); ?>
            ...
            <?php if($currentParent != "index" && $currentChild != "index"){ ?>
</div>
</div>
<div id="footer">
...
<script src="<?php echo base_url(); ?>assets/js/plugins/slimscroll/jquery.slimscroll.min.js"></script>
    ...
    <script src="<?php echo base_url(); ?>assets/js/footable/footable.js" type="text/javascript"></script>
    <!--<script src="<?php echo base_url(); ?>assets/js/footable/footable.sort.js" type="text/javascript"></script>
    <script src="<?php echo base_url(); ?>assets/js/footable/footable.filter.js" type="text/javascript"></script>
    <script src="<?php echo base_url(); ?>assets/js/footable/footable.paginate.js" type="text/javascript"></script>-->
...
foreach($result as $res){
	if(isset($res['CategoryName']) && !empty($res['CategoryName'])){
	...
	$catIdtoName[$res['Id']] = $res['CategoryName'];
	}
}
...
foreach($result as $res){
	...
	$tagCurrentCatId = $res['GroupCategoryId'];
	if(isset($catIdtoName[$tagCurrentCatId]) && !empty($catIdtoName[$tagCurrentCatId]))
	$tagInputForms[] = $catIdtoName[$tagCurrentCatId].' --> '.$res['GroupName'];
}
/*var_dump($tagInputText);
echo '<br><br>';
var_dump($tagInputForms);
exit(0);*/
//$tagInputText = substr($tagInputText,0,-1);
$tagInputText = json_encode($tagInputText);
$tagInputForms = json_encode($tagInputForms);
for($num=1; $num<=5; $num++){
	if($num == 2){
...
'required' => array('label' => 'Required', 'type' => 'checkbox', 'value' => 0),
'id' => array('label' => 'ID / Name', 'type' => 'input', 'value' => 'card_number')));
...
'required' => array('label' => 'Required', 'type' => 'checkbox', 'value' => 0),
'id' => array('label' => 'ID / Name', 'type' => 'input', 'value' => 'card_cvc')));
...
'required' => array('label' => 'Required', 'type' => 'checkbox', 'value' => 0),
'id' => array('label' => 'ID / Name', 'type' => 'input', 'value' => 'card_expiry_month')));
...
'required' => array('label' => 'Required', 'type' => 'checkbox', 'value' => 0),
'id' => array('label' => 'ID / Name', 'type' => 'input', 'value' => 'card_expiry_year')));
...
$inputTag = $e->children(1)->children(0)->tag;
	...
	if($inputTag != 'button' && $inputTag != 'textarea' && $inputTag != 'select' && $inputTag != 'label'){
	...
	$inputRequired = $e->children(1)->children(0)->required;
		...
		if($inputTag == 'select'){
			...
			unset($numoptions);
			unset($tagCategoryAllOptions);
			unset($tagApplyAllOptions);
			for($num=1; $num<=5; $num++){
		...
		} else if($inputTag == 'label'){
			$inputId = $e->children(1)->children(0)->children(0)->name;
			$inputPlaceholder = '';
			$inputPlaceholderOptions = count($e->children(1)->children());
			unset($numoptions);
			unset($tagCategoryAllOptions);
			unset($tagApplyAllOptions);
			for($num=1; $num<=5; $num++){
			...
			for($countOptions=0; $countOptions<$inputPlaceholderOptions; $countOptions++){
				$inputPlaceholder[] = $e->children(1)->children($countOptions)->children(1)->innertext;
				$inputPlaceholderValue[] = $curTagCatString = $e->children(1)->children($countOptions)->children(0)->value;
				$curTagCat = explode(',',$curTagCatString);
				foreach($tagCategory as $tagCateg){
					if($tagCateg['value'] == $curTagCat[0]){
						$tagCategoryAllOptions[$countOptions][] = array('label' => $tagCateg['label'], 'value' => $tagCateg['value'], 'selected' => true);
...
foreach($ISFieldsAllFalse as $ISFieldsAll){
	...
	//var_dump($ISSelectedFields); exit(0);
if($labelFieldtype == "Text input" || $labelFieldtype == "Password input"){
	if($inputId == 'card_number' || $inputId == 'card_cvc' || $inputId == 'card_expiry_month' || $inputId == 'card_expiry_year'){
...
if($labelFieldtype == "Multiple Radios"){
	$LoadedForm[] = array('title' => $labelFieldtype, 'fields' => array(
'label' => array('label' => 'Label Text', 'type' => 'input', 'value' => $labelVal),
...
if($labelFieldtype == "Multiple Checkboxes"){
	$LoadedForm[] = array('title' => $labelFieldtype, 'fields' => array(
'label' => array('label' => 'Label Text', 'type' => 'input', 'value' => $labelVal),
'numoptions' => array('label' => '#of Options', 'type' => 'select', 'value' => $numoptions),
'options' => array('label' => 'Options', 'type' => 'input', 'value' => $inputPlaceholder),
...
.popover.fade.right.in.selectbasic_popup {max-width: none;width: 450px;margin-top: 250px;}
...
.myopen .dropdown-menu{display:block;}
    ...
    <!-- PAGE LEVEL PLUGIN SCRIPTS -->
		...
		tagInputtext = <?php echo $tagInputForms; ?>;
		<?php }else { ?>
		tagInputtext = '';
		<?php  } ?>
		</script>
	...
	$(tab_id).addClass('active').show();
				...
				$('#customer_table').dataTable( {
					...
					"aoColumns":[
										...
										{ "bSortable": false, "mDataProp": "edit" },
										{ "bSortable": false, "mDataProp": "autologin" },
										{ "bSortable": false, "mDataProp": "delete" }
										]
				} );
				...
				$('#sales_table').dataTable( {
					...
					"bServerSide": true,
					"sPaginationType" : 'full_numbers',
					"sAjaxSource": "<?php echo base_url();?>admin/sales/get_sales",
					"fnDrawCallback" : function() {
						showcolumns();
					},
					"aoColumns":[
				...
				$('#leads_table').dataTable( {
					...
					"bServerSide": true,
					"sPaginationType" : 'full_numbers',
					"sAjaxSource": "<?php echo base_url();?>admin/leads/get_leads",
					"fnDrawCallback" : function() {
						showcolumns();
					},
					"aoColumns":[
