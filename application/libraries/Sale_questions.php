...
*    This cart accepts coupons, Two main types:
	...
	Coupon Discount Logic:
		...
		- Maximize the savings for the Users by applying the best discount possible
*/

class sale_questions{
	var $CI;
	...
	private function _init_properties($totals_only=false, $preserve_customer=false)
	{
		...
		if( ! $totals_only) 
		{
			...
			$this->_cart_contents['coupon'] = false;
			$this->_cart_contents['lead'] = false;
			// custom charges
	...
	private function _insert($item)
	{
		...
		else
		{
			...
			if(isset($this->_cart_contents['items'][$newkey]))
			{
				...
				{
					$this->_cart_contents['items'][$newkey]['quantity'] = $this->_cart_contents['items'][$newkey]['quantity'];
				}
			}
			else
			{
...
//			$str = ' - ($subtotal * ('. $reduction_ammount .' /100))';
	...
	********************************************************/
	...
	/*	foreach ($this->_cart_contents['items'] as $item)
		...
		{
			$save['user_id']	= $this->_cart_contents['customer']['id'];
		}
		...
		//$save['user_id']			= $this->_cart_contents['customer']['user_id'];
		/*$save['firstname']			= $customer['user_firstname']; 
		$save['lastname']			= $customer['user_lastname'];
		$save['phone']				= $customer['phone'];
		$save['email']				= $customer['user_email'];
		...
		/*$save['referral'] = $this->CI->session->userdata('referral');
...
////			} 
	...
	{
 	$site_title     = $this->CI->db->get_where('settings', array('code' => 'site_title'))->row()->setting;
		return $site_title;
	}
...
//var_dump($this->_cart_contents['customer']); exit;
	...
	{
		$this->_cart_contents['coupon'] = $data;
		$this->_save_cart();
	}
	
	function save_lead($data)
	{
		$this->_cart_contents['lead'] = $data;
		$this->_save_cart();
	}
	
	function save_order_id($data)
	{
		$this->_cart_contents['last_order_id'] = $data;
		$this->_save_cart();
	}
