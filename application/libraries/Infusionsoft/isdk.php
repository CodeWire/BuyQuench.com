...
class iSDK
{
    ...
    public function getWebFormMap()
    {
		...
		/*$returnFields = array('Id', 'TabName');
		$query = array('FormId' => '-1');
		$DataFormTab = $this->dsQuery("DataFormTab",10,0,$query,$returnFields);
		
		foreach($DataFormTab as $tab){
		 
			$returnFields = array('Id', 'Name');
			$query = array('TabId' => $tab['Id']);
			$groups[] = $this->dsQuery("DataFormGroup",10,0,$query,$returnFields);
		}
		
 		print_r($groups);  exit;*/
		
		$forms =$this->methodCaller("WebFormService.getMap", $carray);
	...
	/* $returnFields = array('Id', 'Name','GroupId');
	 $query = array('FormId' => $id);
	  
	 $DataFormfiled = $this->dsQuery("DataFormField",10,0,$query,$returnFields);
	 print_r($DataFormfiled);  exit;
		*/
		
			$type = $this->getWebFormHtml($id);
