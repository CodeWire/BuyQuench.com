...
function () {
    ...
    return n.Model.extend({
        getValues: function () {
            return t.reduce(this.get("fields"), function (e, n, r) {
				/*if(n["label"] == "MYSELECT" && n["type"] == "select"){
					alert(JSON.stringify(n));
					alert(JSON.stringify(t));
					alert(JSON.stringify(n.value));
				e[r] = t.find(n.value, function (e) {
					if(e.selected){
					alert(JSON.stringify(e.selected));
					}
                    //return e.selected
                }).value;
				//alert(e[r]);
    ...
    return '<div class=\' <% if (name=="option2" || name=="option3" || name=="option4" || name=="option5"){ %> tagoption <% if (value=="") { %>tagGroup <% } %> tagFloat <% } %> tagGroup_<%= name %><% if (name=="option1"){ %> tagoption <% } %> \'><label class="control-label_no"> <%= label %> </label>\n<input class="form-control field" data-type="<%= type %>" type="text" name="<%= name %>" id="<%= name %>" value ="<%= value %>" <% if (name=="option2" || name=="option3" || name=="option4" || name=="option5"){ %> onblur="showTagCatAppy(<%= name %>)"<% } %>  /></div>\n'
}), define("text!templates/popover/popover-select.html", [], function () {
    return '<div class=\'<% if (name=="tagcat1" || name=="tagapply1"){ %> tagcatappy1 tagFloat <% } if (name=="tagcat2" || name=="tagcat3" || name=="tagcat4" || name=="tagcat5" || name=="tagapply2" || name=="tagapply3" || name=="tagapply4" || name=="tagapply5"){ %> <% _.each(value, function(v,k){ if (v["selected"]){ if(v["value"] == "") { %> tagGroup <% } } }); %> tagFloat <% } %> tagGroup_<%= name %>\'><label class=\'control-label_no\'> <%= label %> </label>\n<select <% if (name=="numoptions"){ %> onchange="showButtonOptions(this)" <% } %> class="form-control field " data-type="<%= type %>" id=\'<%= name %>\'>\n<% _.each(value, function(v,k){ %>\n  <option value="<%= v["value"] %>" <% if (v["selected"]){ %> selected <% } %> ><%= v["label"] %></option>\n<% }); %>\n</select></div>\n'
}), define("text!templates/popover/popover-textarea.html", [], function () {
    return "<label class='control-label_no'> <%= label %> </label>\n<textarea class=\"form-control field\" data-type=\"<%= type %>\" style='min-height: 200px' id='<%= name %>'><%= value %></textarea>\n"
}), define("text!templates/popover/popover-textarea-split.html", [], function () {
    return '<div class=\'<% if (name==\"options\"){ %> id_options_tag <% } %>\'> <label class="control-label_no"> <%= label %> </label>\n<textarea class="form-control field" data-type="<%= type %>" style="min-height: 200px" id="<%= name %>"><% for ( var i = 0; i < value.length ; i++ ) { %><%= value[i] %><% if (i < value.length - 1) { %><%= \'\\n\' %><% } %><% } %></textarea></div>\n'
    ...
    return r.extend({
        ...
        mouseDownHandler: function (t) {
            ...
            var n = this;
			var popclass;
			if(this.model.get("title") == 'Select Basic'){
				popclass = 'selectbasic_popup';
			} else {
				popclass = 'popup_field';
			}
            e(".popover").remove(), this.$el.popover("show"), e(".popover").addClass(popclass), e(".popover #save").on("click", this.saveHandler(n)), e(".popover #cancel").on("click", this.cancelHandler(n)), this.model.get("title") !== "Form Name" && e("body").on("mousemove", function (e) {
    ...
    return '<li class="tab_<%= id %>"><a href="#<%= id %>" data-toggle="tab"><%= title %></a></li>\n'
    ...
    return '<h4>Rendered source of your form:</h4>\n<textarea id="render" name="rendered_form" class="span6"></textarea>\n'
